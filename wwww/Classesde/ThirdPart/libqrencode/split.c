...
static int Split_eatAn(const char *string, QRinput *input, QRencodeMode hint)
{
	...
	while(isalnum(*p)) {
		if(isdigit(*p)) {
			...
			dif = QRinput_estimateBitsModeAn((uint32_t)(p - string)) /* + 4 + la */
				+ QRinput_estimateBitsModeNum((uint32_t)(q - p)) + 4 + ln
				- QRinput_estimateBitsModeAn((uint32_t)(q - string)) /* - 4 - la */;
			if(dif < 0) {
				break;
			} else {
				p = q;
...
static int Split_eat8(const char *string, QRinput *input, QRencodeMode hint)
{
	...
	while(*p != '\0') {
		...
		if(mode == QR_MODE_NUM) {
			...
			dif = QRinput_estimateBitsMode8((uint32_t)(p - string)) /* + 4 + l8 */
				+ QRinput_estimateBitsModeNum((uint32_t)(q - p)) + 4 + ln
				- QRinput_estimateBitsMode8((uint32_t)(q - string)) /* - 4 - l8 */;
			if(dif < 0) {
				break;
			} else {
				p = q;
			...
			dif = QRinput_estimateBitsMode8((uint32_t)(p - string)) /* + 4 + l8 */
				+ QRinput_estimateBitsModeAn((uint32_t)(q - p)) + 4 + la
				- QRinput_estimateBitsMode8((uint32_t)(q - string)) /* - 4 - l8 */;
			if(dif < 0) {
				break;
			} else {
