...
- (void)setup
{
    ...
    _selectedTagColor = [UIColor redColor];
    _tagButtonMargin = 5;
    _tagCornerRadius = 5;
    _borderWidth = 0;
    _borderColor = _tagColor;
    _selectedBorderColor = _selectedTagColor;
    _tagListCols = 4;
    _scaleTagInSort = 1;
    _kSelectNum = 0;
    _isFitTagListH = YES;
...
- (void)setScaleTagInSort:(CGFloat)scaleTagInSort
{
    if (_scaleTagInSort < 1) {
        @throw [NSException exceptionWithName:@"TagListView Error" reason:@"(scaleTagInSort)缩放比例必须大于1" userInfo:nil];
    }
...
- (void)addTags:(NSArray *)tagStrs
{
    if (self.frame.size.width == 0) {
        @throw [NSException exceptionWithName:@"TagListView Error" reason:@"先设置标签列表的frame" userInfo:nil];
    }
...
- (void)addTag:(NSString *)tagStr
{
    ...
    [tagButton setTitleColor:_selectedTagColor forState:UIControlStateSelected];
...
- (void)clickTag:(TagButton *)button
{
    ...
    if(button.selected==YES){
        ...
        [button setBackgroundColor:_selectedTagBackgroundColor];
        _kSelectNum++;
    ...
    if (_kSelectNum > self.canTouchNum) {
        [self bettonSetNormal:button];
        _kSelectNum--;
        if (_showAlertBlock) {
            [button setBackgroundColor:_tagBackgroundColor];
            _showAlertBlock([NSString stringWithFormat:@"最多只能选%ld个",(long)self.canTouchNum]);
        }
        return;
    }
    if (button.selected) {
    ...
    if (!button.selected) {
        if (_clickCancelTagBlock) {
            _clickCancelTagBlock(button.currentTitle);
        }
    }
    
}

- (void)bettonSetSelected:(TagButton *)button{
    button.selected = YES;
    button.layer.borderColor = _selectedBorderColor.CGColor;
}

- (void)bettonSetNormal:(TagButton *)button{
    button.selected = NO;
    button.layer.borderColor = _borderColor.CGColor;
}
...
- (void)setTagSelected:(NSMutableArray *)selectedTags{
    ...
    for (TagButton *tagButton in self.tagButtons) {
        [self bettonSetNormal:tagButton];
    }
    _kSelectNum = 0;
    for (TagButton *tagButton in self.tagButtons) {
        for (NSString * tag in selectedTags) {
            if ([tag isEqualToString:tagButton.titleLabel.text]) {
                [self bettonSetSelected:tagButton];
                _kSelectNum ++;
            }
        }
    }
...
{
    // 获取对应的标题按钮
    TagButton *button = self.tags[tagStr];
...
- (void)setupTagButtonCustomFrame:(UIButton *)tagButton preButton:(UIButton *)preButton extreMargin:(BOOL)extreMargin
{
    ...
    CGFloat btnW = extreMargin?titleW + 2 * _tagButtonMargin+10 : tagButton.bounds.size.width ;
