...
-(void)popViewAtSuperView:(UIView *)superView
{
    ...
    else if (_direction==WBArrowDirectionDown1)
    {
        ...
        [UIView animateWithDuration:0.5 animations:^{
            self.alpha = 1;
            self.backView.frame = CGRectMake(self.origin.x-KScreenScale(200), self.origin.y-5, self.width,-self. height);
            if (self.needCenter) {
                self.backView.center=CGPointMake(self.center.x, self.backView.center.y);
            }
        }completion:^(BOOL finished) {
